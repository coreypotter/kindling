 ...
 * @return void
 */
function kindling_breadcrumb_trail( $args = array() ) {
	...
	if ( function_exists( 'yoast_breadcrumb' ) && current_theme_supports( 'yoast-seo-breadcrumbs' ) ) {
		...
		if ( $breadcrumbs_position = get_theme_mod( 'kindling_breadcrumbs_position' ) ) {
	...
	if ( !is_object( $breadcrumb ) )
		$breadcrumb = new Kindling_Breadcrumb_Trail( $args );
 ...
 * @access public
 */
class Kindling_Breadcrumb_Trail {
	...
	/**
	 * Sets up the breadcrumb trail properties.  Calls the `Kindling_Breadcrumb_Trail::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see Kindling_Breadcrumb_Trail::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see Kindling_Breadcrumb_Trail::set_post_taxonomy()
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			// Postion class
			$p_class = '';
			if ( '' != get_theme_mod( 'kindling_breadcrumbs_position' ) ) {
				$p_class = ' position-' . get_theme_mod( 'kindling_breadcrumbs_position' );
			}

			// Wrap the breadcrumb trail.
			$breadcrumb = sprintf(
				'<%1$s role="navigation" aria-label="%2$s" class="site-breadcrumbs kindling-clr'. $p_class .'" itemprop="breadcrumb">%3$s%4$s%5$s</%1$s>',
	...
	protected function set_labels() {
		...
		$defaults = array(
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'kindling' ),
			'home'                => esc_html__( 'Home',                                  'kindling' ),
			'error_404'           => esc_html__( '404 Not Found',                         'kindling' ),
			'archives'            => esc_html__( 'Archives',                              'kindling' ),
			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'              => esc_html__( 'Search results for &#8220;%s&#8221;',   'kindling' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'kindling' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'kindling' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'kindling' ),
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'kindling' ) ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'kindling' ) ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'kindling' ) ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'kindling' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'kindling' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'kindling' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'kindling' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'kindling' ) ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'kindling' ) ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'kindling' ) ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'kindling' ) ) ) );
